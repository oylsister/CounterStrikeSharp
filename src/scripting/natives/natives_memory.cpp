...
ValveFunction* CreateVirtualFunctionBySignature(ScriptContext& script_context)
{
    ...
    auto function = GetOrCreateValveFunction(function_addr, CONV_CDECL, args, return_type);
...
ValveFunction* CreateVirtualFunction(ScriptContext& script_context)
{
    ...
    auto function = GetOrCreateValveFunction(function_addr, CONV_THISCALL, args, return_type);
...
REGISTER_NATIVES(memory, {
    ...
    ScriptEngine::RegisterNativeHandler("MEM_ALLOC_ALLOCATE_ALIGNED", Native_MemAlloc_AllocateAligned);
